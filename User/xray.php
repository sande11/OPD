        ...
        <section>
            ...
            <div class="right-div">
                <div id="main">
                    ...
                    <div class="head">
                        <div class="col-1">
                            <p class="nav-title">X-Ray Results</p>
                        ...
                        $sql_patient = "SELECT patient_records.patient_id, patient_records.name, xray_test_orders.order_id
                FROM patient_records
                INNER JOIN xray_test_orders ON patient_records.patient_id = xray_test_orders.patient_id
                WHERE patient_records.status IN ('Xray')";
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $sql_status = "SELECT test_status FROM xray_test_orders WHERE patient_id = '$patient_id'";
                                $result_status = mysqli_query($conn, $sql_status);
                                $row_status = mysqli_fetch_assoc($result_status);
                                $patient_status = $row_status['test_status'] ?? 'Processing';
                            ...
                            if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["patient_id"], $_POST["diagnosis"], $_POST["prescription"], $_POST["quantity"])) {
                                ...
                                if (mysqli_query($conn, $sql)) {
                                    ...
                                    echo "<script>window.location.href = 'xray.php';</script>"; // Redirect back to the doctor's office page
                                }
                            }
                        } else {
                        ...
                        <script>
                            function openEditForm(orderId) {
                                const url = `./updatedxrayresults.php?order_id=${orderId}`;
                                const width = 1200;
                                const height = 700;
